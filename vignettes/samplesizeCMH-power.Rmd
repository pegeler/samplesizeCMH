...
and then vectorize it over `theta`. Passing in a vector of thetas will then
provide us with a vector of computed powers. Although a simple `for` loop would
suffice to generate a power curve, there is an additional benefit associated
with the creation of a partially applied function. We will see later how it can
be used to compute the exact effect size that will yield 90% power.

```{r contraceptives-effect.size}
# Precomputing several counts, also for readability
obs_control_exposed <- contraceptives[1, 2, ]
obs_control_total <- apply(contraceptives[, 2, ], 2, sum)
obs_total_by_age <- apply(contraceptives, 3, sum)
...
first_over <- which.min(powers <= 0.9)
interval <- thetas[c(first_over - 1, first_over)]
uniroot(function(theta) partial_power_func(theta) - 0.90, interval)$root
```

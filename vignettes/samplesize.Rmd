...
Starting with the example first used by @Woolson1986 and then revisited by @Nam1992, we will explore the usage of the central function of the `samplesizeCMH`{.r} package, the `samplesize.cmh.test()`{.r} function.
...
We will go over several calculation methods to compare the usage of the `samplesize.cmh.test()`{.r} function for different data sets and experimental designs.
...
sample_size_unstratified <- samplesize.cmh.test(
...
sample_size_binomial <- samplesize.cmh.test(
  p2 = c(0.75,0.70,0.65,0.60),
  theta = 3,
  power = 0.9,
  t = c(0.10,0.40,0.35,0.15),
